...
func (s *Service) UserWithDraw(r *http.Request, user *storage.User, withdrawRequest WithdrawRequest) error {
	...
	if err != nil {
		return StatusUnprocessableEntityError
	}
	if intOrder <= 0 {
	...
	if err != nil {
		return errors.Join(InternalServerError, fmt.Errorf("user id:`%d` could not begin transaction. err: %v", user.ID, err))
	}
	var balance int
	...
	if err != nil && !errors.Is(err, pgx.ErrNoRows) {
		...
		return errors.Join(InternalServerError, err)
	}
	...
	if newBalance < 0 {
		...
		return errors.Join(PaymentRequiredError, err)
	}
